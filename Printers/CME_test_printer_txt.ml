...
let print_incoming_msg (s : feed_state) = 
	let ref_a_str  = ref_packet_to_str  (s.channels.ref_a.r_unproc_packets) in 
	let ref_b_str  = ref_packet_to_str  (s.channels.ref_b.r_unproc_packets) in 
	let snap_a_str = snap_packet_to_str (s.channels.snap_a.s_unproc_packets) in 
	let snap_b_str = snap_packet_to_str (s.channels.snap_b.s_unproc_packets) in 

	Printf.sprintf "Refresh_a:\n%s\nRefresh_a:\n%s\nSnapshot_a:\n%s\nSnapshot_b:\n%s\n" 
	(if ref_a_str = "" then "Empty" else ref_a_str)
	(if ref_b_str = "" then "Empty" else ref_b_str)
	(if snap_a_str = "" then "Empty" else snap_a_str)
	(if snap_b_str = "" then "Empty" else snap_b_str)
;;
...
let cme_test_printer ( s : feed_state ) = 
	...
	let str_incoming_data = print_incoming_msg (s) in 
