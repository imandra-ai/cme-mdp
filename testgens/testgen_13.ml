...
let search_space_to_list x = [
    ...
    ExchangeAction(ST_DataSendSnap);
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Multi,4,OrdSell,x.oc_new_qty_58)));
    ...
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Implied,4,OrdSell,x.oc_new_qty_48)));
    ExchangeAction(ST_DataSendInc);
    ExchangeAction(ST_Snapshot(SecA));
    ExchangeAction(ST_DataSendSnap);
    ExchangeAction(ST_Snapshot(SecB));
...
let write_jsons m =
    ...
    packets |> packets_to_json
            ...
            |> Yojson.Basic.to_file (Printf.sprintf "generated/test_b51799029277ab157a42b8227a142dea_%d.json" !n) 
;;
:shadow on
:adts on
