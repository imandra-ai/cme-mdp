...
let search_space_to_list x = [
    ...
    ExchangeAction(ST_DataSendSnap);
    BookAction(ST_Delete(make_ord_del_data(SecB,Book_Type_Implied,OrdSell,x.od_level_num_86)));
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Implied,3,OrdBuy,x.oc_new_qty_45)));
    BookAction(ST_Change(make_ord_change_data(SecB,Book_Type_Multi,1,OrdBuy,x.oc_new_qty_71)));
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Multi,4,OrdBuy,x.oc_new_qty_57)));
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Implied,1,OrdSell,x.oc_new_qty_42)));
    BookAction(ST_Change(make_ord_change_data(SecB,Book_Type_Implied,3,OrdSell,x.oc_new_qty_66)));
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Multi,3,OrdSell,x.oc_new_qty_56)));
    BookAction(ST_Change(make_ord_change_data(SecB,Book_Type_Implied,5,OrdBuy,x.oc_new_qty_69)));
    ExchangeAction(ST_DataSendInc);
    ExchangeAction(ST_Snapshot(SecA));
    ExchangeAction(ST_DataSendSnap);
    ExchangeAction(ST_Snapshot(SecB));
...
let write_jsons m =
    ...
    packets |> packets_to_json
            ...
            |> Yojson.Basic.to_file (Printf.sprintf "generated/test_637096f3be8dff4ed3c021af93ecd0b9_%d.json" !n) 
;;
:shadow on
:adts on
