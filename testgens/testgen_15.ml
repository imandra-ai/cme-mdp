...
let search_space_to_list x = [
    ...
    ExchangeAction(ST_DataSendSnap);
    BookAction(ST_Change(make_ord_change_data(SecB,Book_Type_Implied,5,OrdSell,x.oc_new_qty_70)));
    BookAction(ST_Change(make_ord_change_data(SecB,Book_Type_Multi,3,OrdBuy,x.oc_new_qty_75)));
    BookAction(ST_Change(make_ord_change_data(SecB,Book_Type_Multi,1,OrdBuy,x.oc_new_qty_71)));
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Implied,5,OrdSell,x.oc_new_qty_50)));
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Implied,5,OrdBuy,x.oc_new_qty_49)));
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Multi,2,OrdBuy,x.oc_new_qty_53)));
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Multi,1,OrdBuy,x.oc_new_qty_51)));
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Multi,5,OrdSell,x.oc_new_qty_60)));
    ExchangeAction(ST_DataSendInc);
    ExchangeAction(ST_Snapshot(SecA));
    ExchangeAction(ST_DataSendSnap);
    ExchangeAction(ST_Snapshot(SecB));
...
let write_jsons m =
    ...
    packets |> packets_to_json
            ...
            |> Yojson.Basic.to_file (Printf.sprintf "generated/test_d7e9659c459bc66394c685b36469dce6_%d.json" !n) 
;;
:shadow on
:adts on
