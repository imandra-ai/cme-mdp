...
let search_space_to_list x = [
    ...
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Implied,4,OrdBuy,x.oc_new_qty_47)));
    ...
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Multi,1,OrdSell,x.oc_new_qty_52)));
...
let write_jsons m =
    ...
    packets |> packets_to_json
            ...
            |> Yojson.Basic.to_file (Printf.sprintf "generated/test_9acc0ee7ce15ba0abb1f95cbff1c30c1_%d.json" !n) 
;;
:shadow on
:adts on
