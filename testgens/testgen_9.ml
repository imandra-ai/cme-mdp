...
let search_space_to_list x = [
    ...
    BookAction(ST_Delete(make_ord_del_data(SecB,Book_Type_Implied,OrdBuy,x.od_level_num_85)));
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Multi,5,OrdBuy,x.oc_new_qty_59)));
    BookAction(ST_Change(make_ord_change_data(SecB,Book_Type_Multi,2,OrdSell,x.oc_new_qty_74)));
    BookAction(ST_Change(make_ord_change_data(SecB,Book_Type_Implied,1,OrdBuy,x.oc_new_qty_61)));
    BookAction(ST_Change(make_ord_change_data(SecB,Book_Type_Implied,2,OrdSell,x.oc_new_qty_64)));
...
let write_jsons m =
    ...
    packets |> packets_to_json
            ...
            |> Yojson.Basic.to_file (Printf.sprintf "generated/test_800a99c9b0871ce1d6d42b6d2e88cd39_%d.json" !n) 
;;
:shadow on
:adts on
