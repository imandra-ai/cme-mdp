...
let search_space_to_list x = [
    ...
    BookAction(ST_Change(make_ord_change_data(SecB,Book_Type_Implied,1,OrdBuy,x.oc_new_qty_61)));
    BookAction(ST_Delete(make_ord_del_data(SecB,Book_Type_Implied,OrdSell,x.od_level_num_86)));
    BookAction(ST_Change(make_ord_change_data(SecB,Book_Type_Multi,5,OrdBuy,x.oc_new_qty_79)));
    BookAction(ST_Change(make_ord_change_data(SecB,Book_Type_Multi,1,OrdBuy,x.oc_new_qty_71)));
    BookAction(ST_Change(make_ord_change_data(SecA,Book_Type_Multi,2,OrdSell,x.oc_new_qty_54)));
    BookAction(ST_Change(make_ord_change_data(SecB,Book_Type_Multi,4,OrdSell,x.oc_new_qty_78)));
...
let write_jsons m =
    ...
    packets |> packets_to_json
            ...
            |> Yojson.Basic.to_file (Printf.sprintf "generated/test_78e99e45669d4fc998a21bf6f0ee5542_%d.json" !n) 
;;
:shadow on
:adts on
