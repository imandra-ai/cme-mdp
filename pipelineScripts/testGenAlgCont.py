...
let   run_all actions = run   ( start_state, search_space_to_list actions);;
let valid_all actions = valid ( start_state, search_space_to_list actions);;
...
if __name__ == "__main__":
    ...
    send_snaps =               Sequence( constructors=["ExchangeAction", "ST_Snapshot", "SecA" ] )
    send_snaps = send_snaps >> Sequence( constructors=["ExchangeAction", "ST_DataSendSnap"     ] )
    send_snaps = send_snaps >> Sequence( constructors=["ExchangeAction", "ST_Snapshot", "SecB" ] )
    send_snaps = send_snaps >> Sequence( constructors=["ExchangeAction", "ST_DataSendSnap"     ] )
    ...
    data = sample( send_snaps >> 20*modify_book >> send_snaps )
